...
- 最近验证：完整测试 `124 passed, 2 skipped`；浏览器验证结构化错误提示无溢出、无控制台错误，测试 Key 已清除。
...
2. 为设置持久化补充异常场景测试（损坏的 `localStorage`、旧版本字段迁移、保存失败）。
3. 对多论文混合失败增加按论文/原因汇总；当前任务级提示采用运行报告中的首个具体错误。
